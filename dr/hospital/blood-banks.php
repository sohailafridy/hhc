...
if ($filter_city > 0) {
    $where .= " AND bb.city_id = $filter_city";
}
...
.blood-bank-card .card-body {
    padding: 20px;
}

.blood-bank-card .bb-icon {
    width: 50px;
    height: 50px;
    border-radius: 50%;
...
.blood-bank-card .bb-city {
    font-size: 0.85rem;
    color: #64748b;
    margin-bottom: 8px;
}

.blood-bank-card .bb-contact {
    font-size: 0.85rem;
    color: #1e293b;
}

.blood-bank-card .bb-rating {
    display: flex;
    align-items: center;
    gap: 6px;
    margin-top: 10px;
...
.blood-bank-card .bb-rating .stars {
    color: #f59e0b;
    font-size: 0.85rem;
}

.blood-bank-card .bb-rating .count {
    color: #64748b;
    font-size: 0.8rem;
}

.blood-bank-card .bb-status {
...
.bb-status.active { background: #d1fae5; color: #065f46; }
.bb-status.inactive { background: #fee2e2; color: #991b1b; }
...
@media (max-width: 768px) {
    ...
    .blood-bank-grid { grid-template-columns: 1fr; }
}
</style>
...
<div class="content-wrapper">
        ...
        <form method="GET" action="" id="filterForm">
            <div class="filter-row">
                ...
                <div class="filter-group" style="flex: 0 0 auto; display: flex; gap: 8px;">
                    ...
                    <a href="<?php echo BASE_URL; ?>hospital/blood-banks" class="btn-reset">
    ...
    <?php if (mysqli_num_rows($result) > 0): ?>
        <div class="blood-bank-grid">
            <?php while ($bb = mysqli_fetch_assoc($result)): 
                $avg_rating = $bb['avg_rating'] ? round($bb['avg_rating'], 1) : 0;
            ?>
                <div class="blood-bank-card">
                    <div class="card-body">
                        ...
                        <div class="bb-name"><?php echo htmlspecialchars($bb['bb_name']); ?></div>
                        <div class="bb-city"><i class="fas fa-map-marker-alt me-1"></i> <?php echo htmlspecialchars($bb['city_name']); ?></div>
                        <?php if (!empty($bb['bb_contact'])): ?>
                            <div class="bb-contact"><i class="fas fa-phone me-1"></i> <?php echo htmlspecialchars($bb['bb_contact']); ?></div>
                        <?php endif; ?>
                        <div class="bb-rating">
                            <span class="stars">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                            ...
                            <span class="count">(<?php echo $bb['total_reviews'] ?? 0; ?>)</span>
                        </div>
                        <div class="mt-2">
                            <span class="bb-status <?php echo $bb['ustatus'] == 1 ? 'active' : 'inactive'; ?>">
                                <?php echo $bb['ustatus'] == 1 ? 'Active' : 'Inactive'; ?>
                            </span>
                        </div>
                    </div>
                </div>
    ...
    <?php else: ?>
        <div class="no-records">
            ...
            <p>No blood banks match your search criteria.</p>
        </div>
    <?php endif; ?>
